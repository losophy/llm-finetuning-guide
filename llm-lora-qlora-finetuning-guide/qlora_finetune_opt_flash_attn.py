...
# QLoRA 第 2 步：带量化加载 + Flash Attention 2
model_name = "facebook/opt-125m"
model_qlora = AutoModelForCausalLM.from_pretrained(
    model_name,
    quantization_config=bnb_config,
    attn_implementation="sdpa",             # PyTorch 内置 SDPA 内核，无需装 flash-attn（FA2 在 Windows 只能源码编译，且 125M 模型收益≈0）
    dtype=torch.bfloat16,                   # transformers 5.x 已弃用 torch_dtype，改用 dtype
    device_map="auto",
)
...
print("模型以 4-bit 精度加载 (SDPA 注意力已启用)")
...
qlora_args = SFTConfig(
    output_dir="./opt-125m-qlora-flash-attn",
    num_train_epochs=1,
...
qlora_trainer.train()
print("QLoRA + Flash Attention 2 训练完成!")
